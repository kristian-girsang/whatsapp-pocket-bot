...
async function parseViaGemini(config, messageText) {
  const systemPrompt = [
    ...
    'Fix typos in output fields silently, but keep original meaning.',
    'Never change food words into medicine words unless the user clearly means medicine.',
  ...
  if (!parsed) {
    throw new Error('Gemini tidak mengembalikan JSON yang bisa diparse.');
  }
...
async function parseTransaction(config, messageText) {
  ...
  const llmData = await parseViaGemini(config, messageText);
  const normalized = {
